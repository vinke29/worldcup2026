...
export default function GoogleButton({ label = "Continue with Google", setupQuery }: { label?: string; setupQuery?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="w-full flex items-center justify-center gap-3 rounded-xl transition-all hover:opacity-90 active:scale-[0.98] cursor-pointer"
      style={{
        backgroundColor: "#0F2411",
        border: "1px solid #2C4832",
        padding: "12px 24px",
        fontFamily: "inherit",
        fontSize: "14px",
        fontWeight: 600,
